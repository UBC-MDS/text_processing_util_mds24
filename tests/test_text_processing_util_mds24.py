...
def test_text_clean_invalid_input():
    # raise error if calling function with wrong
    # input type did not raise TypeError
...
def test_frequency_vectorizer_list_num_punctuation():
    ...
    np.testing.assert_array_equal(result_tf_matrix, np.empty((3,0))), "Expected an empty array for a list with only numbers and punctuations."
    np.testing.assert_array_equal(result_feature_names, np.array([])), "Expected an empty array for feature names when the list has only numbers and punctuations."
...
def test_tfidf_vectorizer_multiple_docs():
    ...
    tfidf_matrix, feature_names = tfidf_vectorizer(docs)
    assert len(tfidf_matrix) == 2, "Unexpected number of rows in tfidf_matrix"
    assert len(feature_names) > 0, "Expected non-empty list for feature names"
